...
if (typeof persistenceDir === "string") {
    ...
    persistence = {
        ...
        bindState: async (docName, ydoc) => {
            ...
            const yUsers = persistedYdoc.getArray("users");
...
exports.cloneDoc = async (originalRoomId, newRoomId) => {
    try {
        const originalDoc = getYDoc(originalRoomId, true);
        await new Promise((resolve) => setTimeout(resolve, 100));
        const encodedState = Y.encodeStateAsUpdate(originalDoc);

        const newDoc = getYDoc(newRoomId, true);
        ...
        newDoc.share.delete(`tl_${originalRoomId}`);

        const updateStates = Y.encodeStateAsUpdate(newDoc);
        Y.applyUpdate(newDoc, updateStates);
